...
## Модели и параметры
- OpenAI/Yandex используют параметры размера (`size`) и качества (`quality`).
- Gemini Images использует `aspect_ratio` и `image_size` (доступно для `gemini-3-pro-image-preview`).
- В UI отображаются только параметры, релевантные выбранной модели.
