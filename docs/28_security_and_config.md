...
- Все логи формируются в формате JSON с ключами `event`, `level`, `logger`, `correlation_id`.
- `RequestContextMiddleware` пробрасывает `X-Correlation-ID` из запроса и добавляет `user_id`; сервисы обязаны дополнять `project_id` и `story_id` по возможности.
- Логируйте ключевые события: аутентификация, фоновые задачи, публикации, генерация изображений.
...
- Вынесите в `.env` или отдельный конфиг параметры:
  ...
  - Пути к приватному checkout: `PAPERBIRD_PRIVATE_DIR`, `PAPERBIRD_PRIVATE_ARTIFACTS_DIR`.
...
- Для персональных пресетов и экспортов держите отдельный приватный репозиторий
  (например, `../paperbird-private`) и выгружайте артефакты туда командой
  `python manage.py export_private_artifacts <username>`.
...
## 9. Автоматические проверки перед коммитом
- В репозитории используется `.pre-commit-config.yaml` для блокировки секретов и локальных файлов.
- Обязательные локальные шаги:
  1. `pip install pre-commit`
  2. `pre-commit install`
  3. `pre-commit run --all-files`
- Хуки блокируют коммит при попытке добавить:
