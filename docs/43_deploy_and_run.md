...
4. Скопируйте `infra/.env.example` в `infra/.env` и заполните значения (см. раздел 3).

## 3. Конфигурация `.env`

Файл переменных окружения хранится в `infra/.env`. Пример можно взять из `infra/.env.example`.
...
- **Переменные окружения не подхватываются:** убедитесь, что файл `infra/.env` создан и доступен процессу Django, либо задайте переменные напрямую в окружении.  
