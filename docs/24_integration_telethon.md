...
- Команда `python manage.py collect_posts <username>` запускает сбор постов для всех активных проектов пользователя. Поддерживаются флаги `--project <id>` для выборочного сбора и `--follow --interval 30` для непрерывного мониторинга (Ctrl+C для остановки). Если нужно собрать посты за всех пользователей сразу, используйте `python manage.py collect_posts --all-users --limit 50` (флаг `--project` в этом режиме недоступен).
