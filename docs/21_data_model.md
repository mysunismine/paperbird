...
- **image_file** (File): Путь к файлу изображения.
- **prompt** (Text): Промпт для генерации (может быть пустым).
- **source_kind** (Enum): `generated`, `upload`, `source`.
- **is_selected** (Bool): Участвует ли изображение в публикации.
...
- **Story 1:N StoryImage:** Одна история может иметь несколько связанных изображений.
...
- **StoryImage:** Индекс по `story_id` для ускорения выборки изображений по истории.
...
- **Истории (Story), публикации (Publication), изображения (StoryImage):** Хранятся бессрочно, если не удалены пользователем.
