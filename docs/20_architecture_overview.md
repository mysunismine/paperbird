...
- **Очередь задач** — встроенная очередь на моделях `WorkerTask`/`WorkerTaskAttempt` (с поддержкой Celery или RQ при необходимости) для асинхронного выполнения тяжёлых и периодических задач.
...
5. **Фоновые воркеры**: Очередь задач обеспечивает повторные попытки, логирование и управление статусами для Collector/Rewriter/Publisher.
