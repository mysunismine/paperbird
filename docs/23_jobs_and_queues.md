...
### Автоанлок зависших задач

`WorkerRunner` автоматически проверяет задачи в статусе `running`, заблокированные дольше таймаута очереди (`stale_lock_timeout`, по умолчанию 10 минут, для web-коллектора — 5 минут). Если воркер умер и не успел отдать результат, задача возвращается в статус `queued`, а в логах появляется событие `worker_recovered_stale_tasks`. Это избавляет от ручного «разлочивания» через админку и гарантирует, что очередь не остановится из‑за зависшего процесса. Для каждой итерации воркер также пишет `worker_batch_processed` с количеством обработанных задач и числом реанимированных записей — метрика помогает контролировать холостые прогоны.
...
**Мониторинг:** на странице проекта доступны очередь задач и последние ошибки (`/projects/<id>/queues/`).
**Режимы:** 
...
**Очередь:** `collector_web`.  
**Назначение:** универсальный парсер сайтов и RSS-лент на основе JSON-пресетов.  
**Конвейер:** для каждого активного `Source(type=web)` воркер загружает прикреплённый пресет, валидирует его по JSON Schema и последовательно выполняет шаги `fetch → render(optional) → extract → normalize → persist`. Вся логика задаётся декларативно, код остаётся неизменным.  
**Источники данных:** список страниц задаётся через `list_page.seeds`. Для каждого элемента запускается скачивание статьи, нормализация HTML/Markdown, дедупликация (`canonical_url`, `source_url`, `sha256(content)`), и сохранение постов со статусом `new`.  
**Ретраи и защита:** настройки очереди идентичны Telegram-коллектору (до 5 попыток, экспоненциальный бэкофф). При ошибке валидации пресета его статус помечается как `broken`, чтобы исключить источник из следующих запусков.  
**Логирование:** каждая попытка фиксируется в `SourceSyncLog` с количеством созданных и пропущенных материалов. Дополнительно сохраняются `web_last_synced_at` и `web_last_status` в модели источника, а статус можно посмотреть в UI очереди.
- **Параллелизм:** проектная задача теперь лишь распределяет работу — на каждый активный веб-источник создаётся отдельная задача `collector_web` с собственными ретраями. Так один «медленный» сайт не блокирует остальные.
- **Кастомные ретраи:** поля `Source.web_retry_max_attempts/base_delay/max_delay` позволяют точечно задать политику повторов (это особенно полезно для сайтов с радикальными rate-limit). Если поля пустые, используются дефолты очереди.
- **Защита от дубликатов:** перед постановкой новой задачи проверяем, нет ли уже `queued`/`running` задания с тем же `source_id`, чтобы не гонять один сайт параллельно несколько раз.

### Запуск воркеров

- `python manage.py run_worker <queue>` — целевой запуск отдельной очереди.
- `python manage.py run_collectors [--queues collector collector_web]` — объединённый воркер, который обслуживает несколько очередей в одном процессе (по умолчанию Telegram и Web).
