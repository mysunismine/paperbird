...
- **SendCodeUnavailable**  
  Возникает при повторном запросе SMS-кода, когда Telegram исчерпал доступные
  варианты доставки. UI показывает текст «Telegram временно не может отправить
  SMS на этот номер…», мастер предлагает подождать несколько минут или
  продолжить без SMS (код из приложения Telegram). Ошибка логируется с тегом
  `telethon_sms_unavailable`.
