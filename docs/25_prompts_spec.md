...
## 2.1. Переменные в промпте
В тексте промпта можно использовать переменные (токены), которые система автоматически заменит на актуальные значения.

| Токен                     | Описание                                                               |
|--------------------------|------------------------------------------------------------------------|
| `{{PROJECT_NAME}}`       | Название проекта.                                                      |
| `{{PROJECT_DESCRIPTION}}`| Описание проекта.                                                      |
| `{{POSTS}}`              | Список новостей для обработки.                                         |
...
| `{{CURRENT_DATETIME}}`   | Текущая дата и время с учетом часового пояса проекта.                  |
| `{{CURRENT_UTC_OFFSET}}` | Смещение часового пояса проекта относительно UTC (например, `UTC+03:00`). |
| `{{CURRENT_TIMEZONE}}`   | Название часового пояса проекта (например, `Europe/Moscow`).           |
| `{{CURRENT_ISO_DATETIME}}`| Текущая дата и время в формате ISO 8601.                               |

## 2.2. Промпт для идеи изображения
Для генерации рекомендованного промпта изображения используется отдельный шаблон
`image_prompt_template`, который хранится в настройках промптов проекта и
возвращает JSON с полем `prompt`.
...
Вместе с документами в промпт автоматически добавляется **текущая дата и время**, отформатированные согласно настройкам локали и часового пояса проекта. Это позволяет модели использовать актуальную временную информацию при генерации текста.
...
В базе данных пресеты описываются моделью `RewritePreset` и привязываются к конкретному проекту. Пользователь выбирает пресет на странице сюжета, после чего соответствующие инструкции автоматически добавляются к промпту.
