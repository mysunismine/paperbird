...
### Added
- Модуль медиабиблиотеки с загрузкой файлов, импортом медиа из постов и хранением ассетов на уровне проекта.
- Медиа-студия для генерации изображений с повторным использованием рекомендованных промптов из сюжетов.
- Поддержка участников проекта и общий доступ к медиабиблиотеке для всех участников.
- Привязка изображений сюжета к медиабиблиотеке с выбором ассетов из библиотеки.
- Поля «Название» и «Теги» при загрузке медиа для улучшения поиска в библиотеке.
- Массовое редактирование названий и тегов для выбранных медиа в библиотеке.
- Ручной текстовый источник с редактором и сохранением в ленту проекта.
- Политики веб-источников: интервалы, джиттер, лимиты статей, cooldown блокировок.
- Кеширование веб-запросов через `ETag`/`Last-Modified` с условными запросами и 304.
- Инкрементальные маркеры для веб-источников (last seen URL/дата публикации).
- UI-предупреждения о возможной блокировке веб-источников и причинах.
...
- Экран «Чаты» для Telegram-источников с отдельной вкладкой и просмотром диалогов.
- Импорт проекта из JSON/YAML с настройками, промптами и источниками (без постов и медиа).
- История версий для редакторских текстов и отдельный просмотр версии.
- Вкладка «Сервис» в источниках с диагностикой проблемных Telegram-источников и кнопкой обновления метаданных.
...
- В форме фильтрации медиабиблиотеки добавлена подсказка о поиске по названию, промпту и тегам.
- Сообщения из Telegram-чатов исключены из общей ленты и помечаются как «ДИАЛОГ» в промпте.
- В ленте добавлен фильтр по типу источника (новости vs редакторские тексты).
- Добавлен экспорт `telegram_kind` для источников и его восстановление при импорте.
...
- Кнопка «В сюжет» на странице поста теперь создаёт сюжет из текущего поста.
- Дедупликация медиа в карточке поста учитывает разные формы одного и того же URL.
- Пресет `izvestia_zhivotnye` убирает HTML-теги из текста и не дублирует изображения.
- Фильтры типа медиа в библиотеке применяются сразу при выборе.
- Публикация сюжетов с GIF и видео не ломается из-за смешанной отправки медиа.
- Отправка одиночных GIF выполняется как документ, чтобы Telegram не отклонял медиа.
