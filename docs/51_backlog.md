...
- [x] Реализовать автоматическое добавление в промт, который отправляется в gpt-модель, текущую дату. Добавить в настройках проекта локаль и UTC время, чтобы использовать время пользователя.
- [x] Сделать так, чтобы сборщик постов работал за всех имеющихся пользователей сразу.
- [x] Добавить полноценное отображение медиафайлов в постах в ленте проекта http://127.0.0.1:8000/feed/{n}/
- [x] Не работает рерайт с помощью GPT5, проверить правильность кода. При попытке сделать рерайт с помощью gpt5 в UI отображается индикатор ожидания, что идет рерайт, но результата нет. В логах проблемы не видно.
- [x] Добавить возможность добавления исходных медиафайлов в переписанный сюжет. Чтобы можно было его проиллюстрировать.
- [x] Добавить возможность добавления в переписанный текст ссылки на источник, откуда была взята новость. Для этого нужно добавить в промпт вместе с постом его оригинальную ссылку. Чтобы модель при переписывании смогла её использовать.
- [x] Добавить возможность открывать полностью конкретную новость в ленте (detail странцу).
- [x] Файл src/projects/tests.py  слишком большой. Нужно сделать его рефакторинг, разбить на понятные небольшие файлы.
- [x] Файл stories/paperbird_stories/servises.py слишком большой. Код перенесён в пакет
  src/stories/paperbird_stories/services/ (constants, prompts, rewrite, publisher, images,
  helpers, factory), магические числа вынесены в константы.
- [x] Рефакторинг крупных и сложных файлов для улучшения читаемости и поддержки. Выявлены следующие кандидаты:
    - [x] `src/stories/paperbird_stories/views.py`: содержит сложную логику отображения, которую можно разделить на более мелкие компоненты или сервисы.
    - [x] `src/projects/views.py`: аналогично, содержит много логики, которую стоит вынести из представлений.
    - [x] `src/stories/paperbird_stories/tests.py`: очень большой тестовый файл, который следует разбить на несколько файлов, по аналогии с `src/projects/tests/`.
- [x] При создании сюжета http://127.0.0.1:8000/stories/n/ нужно нужно сделать дополнительный этап иллюстраций. на этой странице пользователь может выбрать и добавить в свою публикацию исходные иллюстрации или медиафайлы, они прикрепляются к публикации и публикуются вместе с текстом (как правило следующим постом все вместе).
---
