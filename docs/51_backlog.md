...
- [x] Нужно придумать какой-то новый статус в промпте для своих текстов. Сейчас они подаются как новости и это, скорее всего, не очень удобно.
- [x] Внедрить Codex в workflows git hub action, чтобы он делал code review.
...
- [ ] Поддержка Telegram-чатов с отдельным UI (не показывать в общей ленте):
  - [x] При добавлении источника типа Telegram различать канал vs чат (по метаданным).
  - [x] Сообщения из чатов не показывать в общей ленте постов.
  - [x] Отдельный экран/вкладка «Чаты» с просмотром сообщений.
  - [x] Минимальный UI: группировка по тредам/ответам, автор, время, поиск по сообщениям.
  - [x] Действия: выбрать сообщения и отправить в Story/рерайт как «диалог».
  - [x] Сервисная вкладка в источниках: диагностика unknown-чатов и кнопка обновления метаданных.
---
