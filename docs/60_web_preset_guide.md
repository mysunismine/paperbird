...
- `images` допускает либо одну строку, либо массив строк. Массив удобен, если на странице используются разные типы медиа (обычные `<img>` + фотогалерея с `data-photoview-src`). В этом случае перечислите все выражения, например:
  ```json
  "images": [
    "div.article__body img@src*",
    "div.article__body .photoview__open@data-photoview-src*",
    "header .media img@src*"
  ]
  ```
  Коллектор соберёт результаты каждого селектора, приведёт их к абсолютным ссылкам, уберёт дубликаты и применит настройки `media.images`.
...
- **Фотоленты и галереи размечаются иначе.** Если в верстке есть блоки вида `.photoview__desc-text`, `.gallery__caption` или `data-article-type="photo"`, добавьте их в `selectors.content` (через множественный селектор или контейнер), иначе в ленту попадёт только заголовок. Для изображений перечислите отдельные выражения (`img@src*`, `.photoview__open@data-photoview-src*` и т.п.), чтобы сохранить и превью, и полноразмерные кадры.
