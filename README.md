...
2. Запустите команду:
   ...
   ```
   Параметр `--project <id>` ограничит сбор одним проектом. Для непрерывного мониторинга и «живого» сбора добавьте `--follow` и задайте интервал опроса (секунды):
   ```bash
   python manage.py collect_posts <username> --follow --interval 30
  ```
  Команда будет работать до остановки (Ctrl+C), раз в 30 секунд опрашивая подключённые источники.
