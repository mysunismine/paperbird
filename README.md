...
Базовый каркас локального сервиса на Django 5.1 для работы с данными из Telegram-каналов и лент новостей. Сервис позволяет собирать новости из различных источников в собственную базу данных, обрабатывать их, рерайтить с помощью LLM моделей и публиковать посты в Telegram.
...
Для полного соответствия базовым правилам стоит прогонять и flake8 (использует конфиг `.flake8`,
игнорирует `.venv` и миграции, длина строки 100):
```bash
source .venv/bin/activate
flake8
```
...
- ложные срабатывания фиксируются в `.secrets.baseline` (обновление: `git ls-files -z | xargs -0 detect-secrets scan -n > .secrets.baseline`).
...
Github [@mysunismine](https://github.com/mysunismine)
